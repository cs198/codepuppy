...
int countNeighbor(Grid<string> grid, int r, int c) {
    //this function takes a grid, a row index and a column index as inputs
    //it counts the neighbor of a certain cell that is specified by the two indices

    int count = 0;

    for (int i=max(r-1,0); i<=min(grid.numRows()-1,r+1); i++) {
        for (int j=max(c-1,0); j<=min(grid.numCols()-1,c+1); j++) {
            //max() and min() are used here to make sure not to index out of the bound of the grid
            count += check(grid[i][j]);
        }
    }
    count -= check(grid[r][c]); //minus the cell itself from the count of neighbors
    return count;
}
...
void tick(Grid<string>& grid) {
    ...
    for (int r = 0; r < tempgrid.numRows(); r++) {
        for (int c = 0; c < tempgrid.numCols(); c++) {
            if (countNeighbor(tempgrid,r,c) == 3) {
                grid[r][c]="X";
            }
            else if ((countNeighbor(tempgrid,r,c) <= 1)||(countNeighbor(tempgrid,r,c) >= 4)) {
                grid[r][c]="-";
            }
        }
    }
