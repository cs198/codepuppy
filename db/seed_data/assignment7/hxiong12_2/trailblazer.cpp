 ...
 * Edits: added functionality for if no path exists.*/
...
void backtrackPath (Vector<Vertex*>& path, Vertex* start, Vertex* end) {
    if (start != end && end->previous == NULL) return; //there is no path from start to end
