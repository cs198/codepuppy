...
/*Returns true if a valid word is parsed from the input line.
...
/*Updates the key (queue) and the value (vector) based on the next word read from the document,
 * and stores it in the map of all the n-gram mappings in the document.
...
/*Creates the first key-value pair from the first N words in the document.
 ...
 * Generates individual words using the getWord method and reading the original document.
