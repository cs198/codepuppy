...
void HeapPriorityQueue::checkResize() { //cited from Marty's class lecture code
    if (m_size == capacity) {
        PQEntry* oldH = h;
...
int HeapPriorityQueue::search(string value, int newPriority) {
    for (int i = 1; i <= m_size; i++) {
        if (h[i].value == value) {
            if (h[i].priority < newPriority) throw "Error: new priority cannot be greater than old priority";
...
string HeapPriorityQueue::dequeue() {
    if (m_size == 0) throw "Error: priority queue is empty";
    string s = h[1].value;
    h[1] = h[m_size];
