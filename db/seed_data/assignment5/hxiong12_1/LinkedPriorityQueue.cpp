...
void LinkedPriorityQueue::changePriority(string value, int newPriority) {
    if (front == NULL) { //if list is empty
        ...
        if (current->value == value) { //if element to be changed is at the front of the queue
            if (current->priority < newPriority) throw "cannot downgrade a priority!";
            current->priority = newPriority;
            return;
        }
        while (current->next != NULL) { //if element to be changed is later in the list
            if (current->next->value == value) {
                if (current->next->priority < newPriority) throw "cannot downgrade a priority!";
...
string LinkedPriorityQueue::peek() const {
    if (front == NULL) throw "Empty queue, nothing to see.";
    return front->value;
}
