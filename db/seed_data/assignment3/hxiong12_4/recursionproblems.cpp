...
// Extension for Sierpinski triangle - draws colorful triangles
...
//Problem 4: draws colorful Sierpinski triangles
int chooseColor(int order) {
    int i = order % 7;
    switch (i) {
...
void drawTriangle(GWindow& gw, int x, int y, int size, int color) {
    GPolygon *triangle = new GPolygon();
    triangle->setFillColor(color);
    triangle->addVertex(x, y);
    triangle->addVertex(x + size, y);
    triangle->addVertex(x + size/2, y + size * sqrt(3) / 2);
...
void drawSierpinskiTriangle(GWindow& gw, int x, int y, int size, int order) {
    if (order > 0) {
        int color = chooseColor(order);
        drawTriangle(gw, x, y, size, color);
    ...
    if (!map.containsKey(symbol)) { //if symbol is not contained in the grammar file, the program will print whatever string the user has input
        ...
        for (string key : sentencePart) {
            if (!map.containsKey(key)) { //a terminal element has been reached
                ...
                generateSentence(key, times, sentence, v, map); //continue to find a terminal element
            }
        }
    }
}
...
Vector<string> grammarGenerate(istream& input, string symbol, int times) {
    ...
    //fills in vector with the user input number of symbols
    while (v.size() < times) {
        string sentence = "";
