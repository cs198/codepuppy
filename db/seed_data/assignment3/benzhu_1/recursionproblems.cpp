...
bool isBalanced(string exp) {
    if (exp == "") {
        return true;
    } else if (exp[exp.rfind('(')+1] == ')') { //if the last ( is on the left of a )
...
void floodFill(int x, int y, int width, int height, int color) {
    //base case: goes out of the boundaries
...
void readFile (istream& input, HashMap<string,Vector<Vector<string>>>& map) {
    ...
    //it reads the file and stores the grammar rule into the map
    ...
    while (getline(input, line)) {
        ...
        //so that each rule is a vector of string
...
void generateElement(string symbol, string& y, HashMap<string,Vector<Vector<string>>>& map) {
    //this fuction takes a symbol, string y and the map as inputs
    ...
    //base case: if the symbol is  a terminal (i.e. not a key in the map)
    if (!map.containsKey(symbol)) {
        y = y + " " + symbol;
        return;
    } else {
        //randomly choose one of the rules corresponding to the non-terminal
