        ...
        if(word.length() == 1) return true;
            ...
            for(int i = x - 1; i <= x + 1; i++){
                for(int j = y - 1; j <= y + 1; j++){
                    if(humanWordSearch(word.substr(1), i, j, chosen)) return true;    // explore
                }
            }
            chosen.remove(choice);    // un-choose
...
Set<string> Boggle::humanWords() const {
    return words;
}
...
void Boggle::findWords(int x, int y, string wordSoFar, Set<string>& chosen, Set<string>& result) {
    ...
    if(x >= 0 && x < 4 && y >= 0 && y < 4 && !chosen.contains(choice)){
        ...
        if(boggleDictionary.containsPrefix(wordSoFar)){
            ...
            for(int i = x - 1; i <= x + 1; i++){
                for(int j = y - 1; j <= y + 1; j++){
                    findWords(i, j, wordSoFar, chosen, result);    // explore
                }
            }
            chosen.remove(choice);    // un-choose
        }
