...
bool Boggle::humanWordFindable(const string word){
    ...
    for (int row = 0; row < 4; row ++) {
        for (int col = 0; col < 4; col++) {
            ...
            BoggleGUI::labelCube(row, col, stringToChar(board[row][col]), true);
            ...
            BoggleGUI::labelCube(row, col, stringToChar(board[row][col]), false);
        }
    }
    return false;
}
    ...
    if (index == word.size()) { // base case
        ...
        for (int r = row - 1; r <= row + 1; r++) {
            for (int c = col - 1; c <= col + 1; c++) {
                if (tempBoard.inBounds(r, c) && stringToChar(tempBoard[r][c]) != '?') {
                    BoggleGUI::setAnimationDelay(50);
                    BoggleGUI::labelCube(r, c, stringToChar(board[r][c]), true);
                    ...
                    BoggleGUI::labelCube(r, c, stringToChar(board[r][c]), false);
                }
            }
        }
        return false;
