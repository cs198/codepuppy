...
Boggle::Boggle(Lexicon& dictionary, string boardText) { //initializes graphics window
...
bool Boggle::humanWordSearchHelper(string word,int i, int j, int index) {
        ...
        for (int h = i - 1; h < i + 2; h++) { //EXPLORE all cubes surrounding chosen cube
            for (int k = j - 1; k < j + 2; k++) {
                if (board.inBounds(h, k) && (visited.get(h, k) == false)) { //if cube has not yet been chosen
                    ...
                    if ((board.get(h, k) == word[index]) && humanWordSearchHelper(word, h, k, index + 1)) {
    ...
    for (int i = 0; i < BOGGLE_SIZE; i++) {
        for (int j = 0; j < BOGGLE_SIZE; j++) {
            ...
            if (board.get(i, j) == word[0]) { //if the cube is the first letter of the word
                if (humanWordSearchHelper(word, i, j, 1)) { //starts the search
                    return true;
                    /*NOTE: cannot use "return humanWordSearchHelper;" alone here. For instance, there
                 * might be multiple cubes with the same letter, but the word might not be able to
                 * be formed from all those locations. If the method used "return humanWordSearchHelper;",
                 * it can return false prematurely without testing for all conditions.*/
                }
            }
        }
    }
    ...
    for (int h = i - 1; h < i + 2; h++) { //explore all cubes surrounding the chosen cube
        for (int k = j - 1; k < j + 2; k++) {
            if (board.inBounds(h, k) && (visited.get(h, k) == false)) { //if cube has not been explored yet
                ...
                if (b_dictionary.contains(word)) { //base case: dictionary contains the potential word
...
void Boggle::createBoard() {
    ...
    for (int i = 0; i < BOGGLE_SIZE; i++) { //creates a string with all the shuffled letters in order
        for (int j = 0; j < BOGGLE_SIZE; j++) {
            letters += board.get(i, j);
            visited.set(i, j, false);
        }
    }
    BoggleGUI::labelAllCubes(letters); //updates GUI to display shuffled letters. All letters are labeled fasle.
}
